...
export default function WabaDashboardPage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-900 p-6 space-y-6">
      ...
      {/* ── KPI: Custo por lead / reunião ── */}
      {(() => {
        const totalLeads = data.pipeline.reduce((acc, s) => acc + s.count, 0);
        const costPerLead = totalLeads > 0 ? data.cost.total / totalLeads : 0;
        const costPerMeeting = data.meetings.total > 0 ? data.cost.total / data.meetings.total : 0;
        const conversionRate = totalLeads > 0 ? ((data.meetings.total / totalLeads) * 100) : 0;
        return (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <StatCard
              ...
              value={data.meetings.total > 0 ? fmtCurrency(costPerMeeting) : "—"}
              sub={data.meetings.total}
              subLabel="reuniões marcadas"
