...
export default function ContactsPage() {
  ...
  const fetchContacts = useCallback(async (currentPage: number, searchTerm: string) => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ page: String(currentPage), limit: "20" });
      if (searchTerm) params.set("search", searchTerm);
      const result = await api.get<ContactsResponse>(`/contacts?${params.toString()}`);
  ...
  const fetchOrganizations = useCallback(async () => {
    try {
      const result = await api.get<{ data: Organization[] }>("/organizations?limit=100");
  ...
  const openModal = () => {
    fetchOrganizations();
    setForm({ name: "", email: "", phone: "", position: "", organizationId: "" });
    setModalOpen(true);
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-auto">
      ...
      {/* New Contact Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Novo Contato">
